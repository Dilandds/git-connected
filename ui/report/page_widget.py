...
class PageWidget(QScrollArea):
    ...
    def __init__(self, page: ReportPage, report: Report, is_first: bool,
        ...
        fl.setSpacing(6)
        fl.addWidget(_lbl(t("project.report.page_follow_title"), muted=False, bold=True, size=15))
        fl.addWidget(_sep())
        ...
        pl.setSpacing(8)
        ph_hdr = QHBoxLayout()
        ph_hdr.addWidget(_lbl(t("project.report.page_components"), muted=False, bold=True, size=15))
        ph_hdr.addStretch()
        pl.addLayout(ph_hdr)
        pl.addWidget(_sep())
