"""
Validation screen — per-session validation preparation and reporting.
"""
import logging
...
from PyQt5.QtWidgets import (
    ...
    QFrame, QScrollArea, QSplitter, QLineEdit, QTextEdit,
    QTableWidget, QTableWidgetItem, QHeaderView, QComboBox,
    QGridLayout, QFileDialog, QProgressBar, QSpinBox,
    QDateEdit, QTimeEdit, QAbstractItemView, QSizePolicy, QMessageBox
)
...
class PreparationPanel(QScrollArea):
    ...
    def _build_static(self):
        ...
        hdr = self._stakeholder_table.horizontalHeader()
        ...
        self._milestone_date_edits: List[QDateEdit]  = []
    ...
    def lock(self):
        """Make the entire preparation panel read-only after signing."""
    ...
    def update_cost_summary(self, summary: list, currency: str, target_budget: float = 0.0):
        ...
        # Target budget — from Project Brief "Target total cost" field
        ...
        # Variance — auto-calculated, green = under budget, red = over budget
        var_label = QTableWidgetItem("Variance")
        var_label.setForeground(QColor(_MUTED))
        self._cost_table.setItem(total_row + 2, 0, var_label)
    ...
    def sync_to_session(self):
        """Write UI values back into the current session's data model."""
        if not self._session:
            return
        s = self._session
        null = QDate(2000, 1, 1)
...
class ReportPanel(QScrollArea):
    ...
    def lock(self):
        """Make the entire report panel read-only after signing."""
    ...
    def sync_to_session(self):
        """Write all UI values back into the current session's data model."""
        if not self._session:
            return
        s = self._session
...
class SignatureBar(QWidget):
    ...
    _SIG_BTN = {
        "Approved":         ("#16a34a", "#15803d"),
        "Rejected":         ("#dc2626", "#b91c1c"),
        "Needs correction": ("#d97706", "#b45309"),
    }
    ...
    def set_signature(self, sig: str, locked: bool):
        self._status_lbl.setText(f"Current: {sig}" if sig else "Not signed")
        self._locked_lbl.setText("🔒  Locked" if locked else "")
        for btn in self._btns.values():
            btn.setEnabled(not locked)


# ── Main ValidationWidget ─────────────────────────────────────────────────────
...
class ValidationWidget(QWidget):
    ...
    def _on_signed(self, sig: str):
        dlg = StyledModalDialog(
            self,
            f"Sign as '{sig}'?",
            f"This will permanently lock the session and cannot be undone.\n"
            f"Are you sure?",
        ...
        if dlg.exec_() != dlg.Accepted:
            return
        # Sync before locking so all data is captured
        self._prep_panel.sync_to_session()
        self._report_panel.sync_to_session()
    ...
    def get_data(self) -> dict:
        # Sync UI → model before serialising
        self._prep_panel.sync_to_session()
        self._report_panel.sync_to_session()
