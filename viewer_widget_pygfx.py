...
class STLViewerWidget(QWidget):
        ...
        try:
            ...
            if geom is not None:
                ...
                if positions is not None:
                    ...
                    normals_data = (normals_buf.data if hasattr(normals_buf, 'data') else normals_buf) if normals_buf is not None else None
                    uvs = self._generate_box_uvs(pos_data, normals_data)
                    geom.texcoords = gfx.Buffer(uvs)
                    # Cache base UVs (versioned) so tile-scale slider can reset cleanly
                    geom._base_texcoords_v2 = uvs.copy()
    ...
    def _ensure_texcoords(self, geom, gfx):
        ...
        if positions is None:
            return False
        # Check if texcoords already exist (and were generated by the new bake)
        existing = getattr(geom, 'texcoords', None)
        if existing is not None and getattr(geom, '_base_texcoords_v2', None) is not None:
            return True
        pos_data = positions.data if hasattr(positions, 'data') else positions
        normals_buf = getattr(geom, 'normals', None)
        normals_data = (normals_buf.data if hasattr(normals_buf, 'data') else normals_buf) if normals_buf is not None else None
    ...
    def _reset_and_scale_texcoords(self, mesh_obj, gfx, scale_factor):
        ...
        def _process_geom(geom):
            # Always (re)generate the base via cube projection if the new
            # versioned cache is missing — this also force-busts any legacy
            # planar UVs left over from before the cube-projection fix.
            if not hasattr(geom, '_base_texcoords_v2'):
                ...
                normals_buf = getattr(geom, 'normals', None)
                normals_data = (normals_buf.data if hasattr(normals_buf, 'data') else normals_buf) if normals_buf is not None else None
                geom._base_texcoords_v2 = self._generate_box_uvs(pos_data, normals_data)
            scaled = geom._base_texcoords_v2.copy() * float(scale_factor)
            geom.texcoords = gfx.Buffer(scaled)
    ...
    def update_texture_settings(self, settings):
        ...
        crease_angle = settings.get("crease_angle", 10)
    ...
    def _generate_box_uvs(self, vertices, normals=None):
        ...
        Picks the projection plane based on the dominant axis of each vertex
        normal so curved surfaces don't get streaked like single-plane projection.
        Falls back to the legacy 2-axis planar projection if normals are not
        provided.
        """
        ...
        # Fallback: legacy planar projection if no normals available
        if normals is None:
            spans = size[:3]
        ...
        if norms.ndim != 2 or norms.shape[0] != verts.shape[0] or norms.shape[1] < 3:
            # Bad normals — degrade gracefully to planar projection
            return self._generate_box_uvs(verts, normals=None)

        # Single global world-scale so tile density stays uniform across all
        # 3 projection planes (no visible UV size jumps between cube faces).
        world_scale = float(max(size[0], size[1], size[2]))
        if world_scale <= 0.0:
            world_scale = 1.0

        # Normalised positions in [0, 1] per axis using the same global scale
        # (keeps tiling consistent across faces).
        nx = (verts[:, 0] - mins[0]) / world_scale
        ny = (verts[:, 1] - mins[1]) / world_scale
        nz = (verts[:, 2] - mins[2]) / world_scale

        abs_n = np.abs(norms[:, :3])
        dom = np.argmax(abs_n, axis=1)  # 0=X, 1=Y, 2=Z

        uvs = np.zeros((len(verts), 2), dtype=np.float32)
        # X-dominant -> project to (Y, Z)
        mx = (dom == 0)
        uvs[mx, 0] = ny[mx]
        uvs[mx, 1] = nz[mx]
        # Y-dominant -> project to (X, Z)
        my = (dom == 1)
        uvs[my, 0] = nx[my]
        uvs[my, 1] = nz[my]
        # Z-dominant -> project to (X, Y)
        mz = (dom == 2)
        uvs[mz, 0] = nx[mz]
        uvs[mz, 1] = ny[mz]
