#!/bin/bash
# Build LYNS360 Core edition for macOS
# Usage: ./build_mac_core.sh

set -e

echo "=========================================="
echo "  LYNS360 Core — macOS Build"
echo "=========================================="
...
# Clean previous core build
rm -rf build dist LYNS360-Core-*.dmg

# Build
echo "Building LYNS360 Core .app..."
pyinstaller stl_viewer_mac_core.spec --clean --noconfirm
...
echo "✓ LYNS360 Core .app created successfully"

DMG_NAME="LYNS360-Core-${VERSION}-macOS-${ARCH}.dmg"

# Create DMG (optional)
if command -v create-dmg &> /dev/null; then
    echo "Creating DMG..."
    mkdir -p dist/dmg_temp
    cp -R "dist/LYNS360-Core.app" dist/dmg_temp/

    create-dmg \
        --volname "LYNS360 Core" \
        --window-pos 200 120 \
        --window-size 600 400 \
        --icon-size 100 \
        --app-drop-link 425 175 \
        --icon "LYNS360-Core.app" 175 175 \
        "${DMG_NAME}" \
...
echo ""
echo "=========================================="
echo "  Build complete — Core edition"
echo "=========================================="
