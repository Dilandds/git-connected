#!/bin/bash
# Build LYNS360 Lite edition for macOS
# Usage: ./build_mac_lite.sh

set -e

echo "=========================================="
echo "  LYNS360 Lite — macOS Build"
echo "=========================================="
...
# Clean previous lite build
rm -rf build dist LYNS360-Lite-*.dmg

# Build
echo "Building LYNS360 Lite .app..."
pyinstaller stl_viewer_mac_lite.spec --clean --noconfirm
...
echo "✓ LYNS360 Lite .app created successfully"

DMG_NAME="LYNS360-Lite-${VERSION}-macOS-${ARCH}.dmg"

# Create DMG (optional)
if command -v create-dmg &> /dev/null; then
    echo "Creating DMG..."
    mkdir -p dist/dmg_temp
    cp -R "dist/LYNS360-Lite.app" dist/dmg_temp/

    create-dmg \
        --volname "LYNS360 Lite" \
        --window-pos 200 120 \
        --window-size 600 400 \
        --icon-size 100 \
        --app-drop-link 425 175 \
        --icon "LYNS360-Lite.app" 175 175 \
        "${DMG_NAME}" \
...
echo ""
echo "=========================================="
echo "  Build complete — Lite edition"
echo "=========================================="
